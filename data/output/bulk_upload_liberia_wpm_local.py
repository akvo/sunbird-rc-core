...
BASE_URL = f"{API_DOMAIN}/api/v1"
KEYCLOAK_URL = f"{KEYCLOAK_DOMAIN}/auth/realms/sunbird-rc/protocol/openid-connect/token"

# Sampling configuration
RECORDS_PER_COUNTY = 67  # ~1000 total across 15 counties

# Files (relative to script location)
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
CSV_FILE = os.path.join(SCRIPT_DIR, "liberia_wpm_2017_registration.csv")
LOG_FILE = os.path.join(SCRIPT_DIR, "upload_progress_local.txt")
...
def get_client_secret():
    ...
    CLIENT_SECRET = os.environ.get("DEMO_API_CLIENT_SECRET")
    if not CLIENT_SECRET:
        CLIENT_SECRET = getpass("Enter demo-api client secret: ")
    return CLIENT_SECRET


def sample_equal_per_county(df, records_per_county):
    """Sample equal number of records from each county."""
    sampled_dfs = []
    for county in df["county"].unique():
        county_df = df[df["county"] == county]
        n_samples = min(records_per_county, len(county_df))
        sampled_dfs.append(county_df.sample(n=n_samples, random_state=42))
...
def get_auth_headers():
    ...
    if (
        TOKEN_DATA["access_token"] is None
        or time.time() >= TOKEN_DATA["expires_at"]
    ):
...
def log_result(geo_code, status, message, osid=None, wf_id=None):
    ...
    wf_id_str = wf_id or ""
    log_entry = (
        f"{geo_code}|{status}|{message}|{osid_str}|{wf_id_str}|{timestamp}\n"
    )
...
def row_to_facility(row):
    ...
    facility["location"]["coordinates"] = {
        k: v
        for k, v in facility["location"]["coordinates"].items()
        if v is not None
    }
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            fetch_response = requests.get(
                f"{BASE_URL}/WaterFacility/{osid}", headers=headers, timeout=30
            )
            wf_id = None
            ...
            if retry_count < max_retries:
                ...
                return {
                    "status": "FAILED",
                    "message": f"Auth failed after {max_retries} retries",
                }
...
def bulk_upload(df, batch_size=50):
    ...
    for idx, (_, row) in enumerate(df.iterrows(), 1):
        ...
        if idx % batch_size == 0 or idx == total:
            ...
            print(
                f"\rProgress: {idx}/{total} ({idx*100/total:.1f}%) | "
...
def main():
    ...
    print(
        f"Token obtained (expires at {datetime.fromtimestamp(TOKEN_DATA['expires_at']).strftime('%H:%M:%S')})"
    )
    ...
    # Sample equal records per county
    df_sampled = sample_equal_per_county(df, RECORDS_PER_COUNTY)
    print(f"Sampled {RECORDS_PER_COUNTY} per county: {len(df_sampled)} total")
    print(f"Counties: {df_sampled['county'].nunique()}")
    ...
    df_to_upload = df_sampled[~df_sampled["geo_code"].isin(uploaded_records)]
    ...
    print()
    response = input(
        f"Proceed with uploading {len(df_to_upload)} records? [y/N]: "
    )
    if response.lower() != "y":
        print("Aborted.")
        return
